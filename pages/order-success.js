...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    const storedOrderData = sessionStorage.getItem('orderResult');
    if (storedOrderData) {
      try {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="px-6 py-8">
            ...
            {paymentData && (
              <div className="mb-8">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Informace o platbě</h2>
                <div className="bg-yellow-50 rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <span className="font-medium text-gray-700">Platební metoda:</span>
                      <span className="ml-2 text-gray-900">{paymentData.gateway?.name || paymentData.method}</span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700">Částka:</span>
                      <span className="ml-2 text-gray-900">{paymentData.amount} {paymentData.currency}</span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700">Status:</span>
                      <span className="ml-2 text-gray-900">{paymentData.status}</span>
                    </div>
                    <div>
                  ...
                  {paymentData.instructions && (
                    <div className="mt-4 p-4 bg-white rounded border">
                      <h3 className="font-semibold text-gray-900 mb-2">Pokyny k platbě</h3>
                      <div className="space-y-2 text-sm">
                        <div><strong>Číslo účtu:</strong> {paymentData.instructions.accountNumber}</div>
                        <div><strong>Banka:</strong> {paymentData.instructions.bankName}</div>
                        <div><strong>Částka:</strong> {paymentData.instructions.amount}</div>
                        <div><strong>Variabilní symbol:</strong> {paymentData.instructions.variableSymbol}</div>
                        <div><strong>Specifický symbol:</strong> {paymentData.instructions.specificSymbol}</div>
                        <div><strong>Zpráva:</strong> {paymentData.instructions.message}</div>
                        <div><strong>Splatnost:</strong> {paymentData.instructions.dueDate}</div>
                      </div>
                    </div>
                  )}

                  {/* Payment URL for redirect methods */}
                  {paymentData.paymentUrl && paymentData.redirectRequired && (
                    <div className="mt-4">
                      <a
              ...
              <ul className="space-y-2 text-gray-700">
                ...
                {paymentData && paymentData.instructions ? (
                  <li className="flex items-start">
                    ...
                    Proveďte platbu podle výše uvedených pokynů
                  </li>
                ) : paymentData && paymentData.paymentUrl ? (
                  <li className="flex items-start">
                    <span className="text-blue-500 mr-2">2.</span>
                    Dokončete platbu kliknutím na tlačítko "Dokončit platbu"
                  </li>
                ) : (
                  <li className="flex items-start">
                    <span className="text-blue-500 mr-2">2.</span>
                    Brzy vás budeme kontaktovat ohledně platby
                  </li>
                )}
                <li className="flex items-start">
