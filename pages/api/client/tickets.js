// Client Support Tickets API - Proxy to middleware
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`🔍 Fetching tickets for client ID: ${client_id}`);

    // Call middleware API
    const middlewareUrl = process.env.NEXT_PUBLIC_MIDDLEWARE_URL || 'http://localhost:3005';
    const url = `${middlewareUrl}/api/client/tickets?client_id=${client_id}${limit ? `&limit=${limit}` : ''}`;
    ...
    if (!data.success) {
      return res.status(404).json({
        success: false,
        error: data.error || 'Tickets not found'
      });
    }

    console.log(`✅ Loaded ${data.data.length} tickets for client ${client_id}`);
