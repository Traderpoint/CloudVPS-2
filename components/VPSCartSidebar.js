...
export default function VPSCartSidebar() {
  ...
  const calculateMonthlyPrice = (item) => {
    ...
    if (item.allPrices) {
      ...
      if (monthlyPrice > 0) {
        ...
        console.log(`📊 VPSCartSidebar: ${item.name} - Monthly: ${monthlyPrice} CZK + OS: ${os?.priceModifier || 0} = ${finalPrice} CZK/měs`);
        return finalPrice;
      }
    }
    ...
    console.log(`📊 VPSCartSidebar: ${item.name} - Fallback monthly: ${basePrice} CZK + OS: ${os?.priceModifier || 0} = ${finalPrice} CZK/měs`);
    return finalPrice;
  };
  ...
  const calculatePeriodPrice = (item) => {
    // Use pricing data from middleware API if available
    if (item.allPrices) {
      console.log(`💰 VPSCartSidebar: Using middleware pricing data for ${item.name}:`, item.allPrices);
      ...
      if (periodPrice > 0 && priceField !== 'monthly') {
        ...
        console.log(`📊 VPSCartSidebar: ${item.name} - Period: ${selectedPeriod}, Real period price: ${periodPrice} CZK + OS period: ${osModifierForPeriod} = ${finalPrice} CZK`);
        return finalPrice;
      } else {
        ...
        console.log(`📊 VPSCartSidebar: ${item.name} - Monthly calculation: ${monthlyWithOS} CZK × ${selectedPeriod} = ${finalPrice} CZK`);
        return finalPrice;
      }
    }
    ...
    console.log(`📊 VPSCartSidebar: ${item.name} - Fallback period calculation: ${monthlyPrice} CZK × ${selectedPeriod} = ${finalPrice} CZK`);
    return finalPrice;
  };

  // Calculate total for selected period (not monthly)
  ...
  const getCartPeriodTotal = () => {
    return items.reduce((total, item) => {
      return total + calculateItemPeriodTotal(item);
    }, 0);
  };
  ...
  const getCartMonthlyTotal = () => {
    return items.reduce((total, item) => {
      return total + calculateItemMonthlyTotal(item);
    }, 0);
  };
        ...
        <div className="p-4">
          {items.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 text-sm">Vyberte VPS plán a přidejte jej do košíku</p>
            </div>
          ) : (
            <>
              {/* Cart Items */}
                    ...
                    <div className="pr-6">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          {(() => {
                            // Use real savings if available, fallback to hardcoded discounts
                            ...
                            const fallbackDiscount = periods.find(p => p.value === selectedPeriod)?.discount || 0;
              ...
              {(affiliateId || affiliateCode) && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-4">
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-green-800">Partnerské sledování aktivní</div>
                      {affiliateCode && (
              ...
              <div className="border-t border-gray-200 pt-4 mb-4">
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-gray-900">Celkem za {periods.find(p => p.value === selectedPeriod)?.label}:</span>
                  <span className="text-xl font-bold text-primary-600">{Math.round(getCartPeriodTotal())} Kč</span>
                </div>
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={() => {
                    console.log('🗑️ VPSCartSidebar: Clear cart button clicked');
                    clearCart('manual');
                  }}
      ...
      {items.length > 0 && (
        <div className="mt-4 bg-white rounded-xl shadow-lg border border-gray-100 p-4">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-600 text-xs">Celkové náklady:</span>
              <span className="font-semibold text-primary-600 text-xs">{Math.round(getCartPeriodTotal())} Kč</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 text-xs">Měsíčně:</span>
              <span className="font-semibold text-gray-600 text-xs">{Math.round(getCartMonthlyTotal())} Kč</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 text-xs">Období:</span>
              <span className="font-semibold text-xs">{periods.find(p => p.value === selectedPeriod)?.label}</span>
            </div>
            {(() => {
              // Calculate total real savings for all items
              const getTotalRealSavings = () => {
                ...
                items.forEach(item => {
                  if (item.realSavings) {
                    ...
                    const periodKey = periodMapping[selectedPeriod];
                    const savings = item.realSavings[periodKey];

                    if (savings) {
                      totalSavingsAmount += (savings.amount * item.quantity);
                      totalSavingsPercent = savings.percent; // Use same percent for all items
                      hasRealSavings = true;
                    }
                  }
                });

                return hasRealSavings ? { amount: totalSavingsAmount, percent: totalSavingsPercent } : null;
              };

              const totalRealSavings = getTotalRealSavings();
              const fallbackDiscount = periods.find(p => p.value === selectedPeriod)?.discount || 0;

              const savingsPercent = totalRealSavings?.percent || fallbackDiscount;
              const savingsAmount = totalRealSavings?.amount || 0;

              return savingsPercent > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-600 text-xs">Úspora:</span>
                  <span className="font-semibold text-green-600 text-xs">
                    {totalRealSavings ? (
                      <>{savingsAmount} CZK ({savingsPercent}%)</>
                    ) : (
                      <>{savingsPercent}% sleva</>
                    )}
                  </span>
                </div>
              );
