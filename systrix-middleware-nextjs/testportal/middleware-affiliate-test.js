...
export default function MiddlewareAffiliateTest() {
  ...
  const testAffiliateDetails = async (affiliateId) => {
    ...
    try {
      ...
      // First get affiliate info
      const affiliateResponse = await fetch(`/api/middleware/get-affiliate-products?affiliate_id=${affiliateId}`);
      const affiliateResult = await affiliateResponse.json();

      if (affiliateResult.success) {
        // Get affiliate details from the affiliates list
        ...
        // Combine affiliate info with products
        const combinedResult = {
          ...
          affiliate: {
            ...
            name: affiliate.firstname && affiliate.lastname
              ...
              : affiliate.name || `Affiliate ${affiliateId}`,
        ...
        setSelectedAffiliate(combinedResult);
        addTestResult(`✅ Affiliate details loaded: ${combinedResult.affiliate.name}`, 'success');
        addTestResult(`📦 Found ${affiliateResult.products?.length || 0} products`, 'success');
      } else {
        addTestResult(`❌ Failed to load affiliate details: ${affiliateResult.error}`, 'error');
      }
    } catch (err) {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <p>Komplexní test affiliate systému přes middleware na portu 3010</p>
      ...
      {selectedAffiliate && (
        <div style={{
          ...
          <h3 style={{ margin: '0 0 15px 0', color: '#155724' }}>
            ✅ Affiliate Details: {selectedAffiliate.affiliate?.name || `Affiliate ${selectedAffiliate.affiliate_id}`}
          </h3>
      ...
      <div style={{
        ...
        <ul style={{ margin: 0, paddingLeft: '20px', color: '#383d41' }}>
          ...
          <li>Middleware server must be running on port 3010</li>
